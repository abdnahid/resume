...
| D66 | **Every seeded routing row is flagged `isPlaceholder` and points at the owning head-office section.** | Decided 2026-09-04: the client asked for all 16,399 rows to point at head office until offices enter their own. A stand-in that looks identical to a decision is the failure mode here — the same reasoning as the seeded bank branch details — so the flag travels with the row and the mapping module clears it. |
...
| D99 | **Two fee columns on the parameter, both always filled: `feePoisha` and `urgentFeePoisha`. The urgent price is the sum of the second, exactly as the normal price is the sum of the first — no rule, no doubling at read time, no refusal.** `urgentFeeSource` records where each figure came from. Amends D62. | The client's simplification, and the right one. D62 made `urgentFeePoisha` nullable with null meaning "twice the normal fee", which is a rule evaluated at read time — and the moment a second wing prices differently, a null stops meaning "double" and starts meaning "nobody knows", with no way to tell the two apart. Storing both numbers removes the ambiguity instead of managing it: the column is NOT NULL, the reader sums, and selection (D101) composes for free because both prices are per parameter. **The work moves to the importer, which is where it belongs** — deciding a number once, in a script whose output a person reads, beats deciding it on every page load. Three provenances, all recorded: `doubled` where the wing's own published total is exactly twice its normal total and therefore proves each part doubles — 2,564 of 4,222 chemical rows (60%) plus all 713 textile rows; `apportioned` where the published urgent total is *not* double and the surcharge falls on some tests and not others — 1,591 rows (37%), filled so the package total matches what the wing published, which is right for a whole package and unverifiable per parameter (D100); and `unpublished` for the 67 rows in packages carrying no urgent total at all. Labelling rather than refusing follows D49 over D45: a labelled figure that is right in aggregate beats no urgent service, and the flag is what keeps it from being mistaken for a decision — the same discipline as `priceIsPlaceholder` and `LabRouting.isPlaceholder`. When the wing answers D100, `WHERE urgentFeeSource = 'apportioned'` is the list to correct. |
...
1. **Urgent pricing — imported under the client's rule, and the wing's own
   totals disagree.** Every parameter now carries an explicit urgent fee: 2× the
   normal fee where the urgent turnaround is shorter, the normal fee where it is
   not (D99). That rule reproduces the wing's published urgent total for 271 of
   408 packages and **charges more than the wing's own figure for 115** — Poultry
   Feed comes to 40,000 against a published 25,000. The evidence in D100 says the
   difference is per-test: some tests cannot be expedited and carry no surcharge.
   To confirm, the wing needs to answer whether that is the reason, and if so
   which tests. Correcting it later is one `UPDATE` per named test.
