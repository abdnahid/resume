...
| D57 | **`office_head` is a new role, separate from `officeadmin`.** Whoever holds it receives their office's submitted applications and starts them moving. | Decided 2026-09-02. Payroll authority and file-routing authority are different jobs, and welding them together would mean whoever runs an office's salaries also decides who reviews its licence applications. It is a **role and not a designation** because the client's own point is that when the senior desk is vacant a more junior officer acts in it. `User.role` was a single enum, so one person could not be both office admin and office head — accepted deliberately, and **reversed by D122**: the client's own case is that rarely one person is both, and the single column had already forced `import:office-heads` to displace payroll at 14 of 23 offices. |
...
| D124 | **A designation and a desk are different things and may differ on purpose, so a *guessed* seat is flagged as one.** `Employee.orgPostIsInferred`; `displayDesignation()` trusts the post outright once a seat is confirmed and falls back to the recorded designation while it is not. | The client's model, stated 2026-09-10: normally an Inspector (Metrology) sits on an Inspector (Metrology) desk, but where a post has no exact match somebody with adjacent expertise fills it — an Inspector (Metrology) on an Examiner (Chemical) desk because he knows chemistry. **The post is then the job**, and the section it sits in is where his files go, which is already how routing works. What was missing is that the system could not tell that apart from a mistake: **every desk held today was assigned by a script**, and `import:desks` takes whatever seat at the employee's grade is free when no title matches, which put 325 of 481 on a post of a different rank. A deliberate placement and a guess were identical in the data, which is why `displayDesignation()` had to distrust every rank disagreement — and why an *Assistant Director* on an *Assistant Director (CM)* desk was shown without their section. Flagged: 323 rows where the desk's rank disagrees with the record; 158 where it agrees were left alone, since the answer is the same either way. **176 of 481 now take their title from the desk, against 157 before**, including the client's own example — *Inspector* → *Field Inspector (Metrology)*. The remaining 305 are the correction list. **Nothing can yet clear the flag**: a re-seating screen is the next step, and until it exists `import:desks` is the only writer and marks everything it does as inferred. |
| D125 | **Where several offices can run a test and nothing settles which, the field officer chooses — on the sampling screen, for that consignment.** `ApplicationParameterDestination`; it overrides a standing preference as well as filling a gap. | The last thing between the routing model and a sealed box, and the client hit it on a real file: application 26 at Faridpur, Ceramic Tiles, seven tests placed and **two that Khulna and head office can both run**. `resolveDestinations()` had computed those choices since D116 and `buildPlanFor()` threw them away, so the screen could state the problem and offer no way to answer it. Resolution order is now **chosen → here → preferred → sole**: an explicit choice wins over everything, which is the client's rule that a preference pre-fills and the officer may override. **Per application, not per office** — the same split as `SampleRequirement` against `OfficeSampleRequirement`: this is a decision about one consignment, made by the person who knows Khulna is three days behind this month, and a choice made once under pressure should not silently become policy. The office's standing answer stays `RoutingPreference`, set on the map. Refused unless the office holds the capability, **checked in the service and not only at the button**, and re-checked when the plan is built because a lab may drop a capability in between — a choice that has stopped being possible says so rather than vanishing. Verified on application 26: two choices offered, an incapable office refused by name, then three boxes — Faridpur 7 tests, Khulna 1, head office 1. |
| D126 | **The database is Neon**, moved off Prisma Postgres 2026-09-12 after the account hit a plan limit that refused every connection. Restored from a Studio CSV export by `npm run db:restore`. | Not a considered migration — the database became unreachable, reads included, so `pg_dump` was not available either. A per-model CSV export through the web Studio was the only copy: 95 files, 76 tables, 14,587 rows. It was audited before anything was written — **no table's row count lands on an exact multiple of 500**, so every last page is partial and the pagination ran to completion, and the counts matched what had been measured live the same day. One gap found and closed by re-exporting: `VerdictClause` and `SalaryFixation` referenced a `CaseVerdict` that had not been exported, nor had its parent `EmployeeCase`. Restored into an empty Neon database and verified: **all 76 tables match their CSV exactly**, the 27 hand-entered capability rows are intact across the right four offices, *Physical Lab, Faridpur* survived, application 26 resolves to 3 boxes with 0 open choices and 0 problems, and a fresh insert takes the next id rather than colliding. `DATABASE_URL` is Neon's pooled endpoint; **DDL must use the direct one**. |
| D127 | **Schema changes go through migrations, and `prisma migrate dev` is not used.** `prisma/migrations/0_init` baselines the schema as it stood at the Neon move; `npm run db:migration -- <name>` writes the next one and applies nothing, `npm run db:deploy` applies it. | Recommended since step 3 of this plan and finally cheap to do: the Neon database was created by `db push`, so there was still no history, and a baseline against a database whose schema provably matches the datamodel costs nothing. Verified before marking it applied — `migrate diff --from-config-datasource --to-schema` reported **no difference**, so the 2,711-line baseline is the database rather than an aspiration. **`migrate dev` is deliberately avoided**: it is built for a throwaway development database, can decide the database has drifted and offer to reset it, and wants a shadow database. There is one database here, shared by two machines, holding 731 real employees and a live payroll — nothing in the workflow should be able to drop it. Generate → read → apply is the same discipline as the `--dry` on every importer. The generator **refuses while anything is pending**, because it diffs against the live database and an unapplied migration's changes would otherwise be written into the new one and applied twice; and it **lists the destructive lines** rather than leaving them in the middle of the DDL. Proved by round trip: a probe table added, generated, deployed, reverted, and both records removed so the baseline stands alone. |
| D128 | **The applicant gets one sampling letter per destination office, not one compiled letter covering every journey.** `SampleLetter.officeId` is set on the applicant kind too, and `applicantLettersFor()` returns one per office. | The client found it on a real file: application 26 splits across Dhaka, Faridpur and Khulna, and the single letter described all three journeys on one sheet. That sheet cannot be handed in anywhere — a counter receiving the Khulna box cannot tell which paragraph is theirs, and the applicant cannot leave the paper behind when the box is accepted, because the other two errands are printed on it. One letter per destination is what actually gets carried, checked, signed for and filed. The letter view filters its consignment query by the same office, so a letter can never name a box the reader has no business accepting; `?office=` selects one and an older link with no parameter still resolves to the earliest, which is what a single-box file needs. |
| D129 | **Issuing the letters demands the testing fee, in the same act, and the amount is snapshotted.** `Application.testFeePoisha` + `state: test_fee_demanded`; the box cannot be received until it is paid. | This is the first moment the fee exists as a real number: the sub-products are settled, the destinations chosen and the boxes sealed, so the sum across every laboratory is finally computable (D62). Issuing the letters apart from the demand would mean asking somebody to carry jars against a fee nobody had raised. **The amount is stored, not recomputed** — catalogue prices move and an apportioned urgent fee is corrected when the wing answers D100, and an applicant told ৳4,000 must be charged ৳4,000. **The demand is a state and a figure, not a `Payment` row**: a payment needs a payer and the officer issuing this is not it, so the row is raised when the applicant starts checkout, exactly as the application fee is. `submitConsignment()` refuses while it is unpaid and names the amount, and the counter shows paid/unpaid — **read-only, and always will be** (spec §5.2): a counter that could mark a file paid is a counter that can be argued with. The client's own sequence, and the states say it in their words: *Test fee requested* → *Awaiting sample submission*. |
| D130 | **A sampling letter is read from the letter, not from the file** — `/workflow/letters`, keyed on `addressedToEmployeeId` for a wing head and on `officeId` + the `one_stop` role for a counter. And a letter names an **office**, never a laboratory. | Two faults, one cause. First, `canViewApplication()` grants a reader the file they hold, handled, or head the office of (D80), and the officer a letter is addressed to is **none of those**: a Faridpur inspection sends a box to Khulna, so Khulna's officer is asked to expect samples on a file that will never touch their desk. Every addressee was refused, and the letters existed as rows with no reader — the wing head and the counter were both told they would "get the letter" and neither could open one. Second, `letterRecipientsFor()` returned the destination office id in a field called `labId`, which `issueSampleLetters()` wrote into `SampleLetter.labId` — **a column whose foreign key points at `Lab`**. Office ids run 1–23 inside lab ids 1–46, so every one of them was a valid lab id and the database took it in silence; read back, the letter to Khulna's wing head named *Physical Lab, **Barisal***. The same collision that put the wrong lab on the sealing screen, and the same cure: say what the number is. `officeId` is now set on all three kinds and `labId` is null, which is also correct on its own terms — since D116 the destination is an office, accountable for the testing whether it runs on its own bench or sends it to an accredited outside lab, and the office is what decides. **The wing-head letter is blinded and the counter's is not** (D71): testing-wing staff get the package, the box, the seal and the jar count and never the company, the factory, the brand or the application number, because the variant *is* the applicant's identity; a counter hands the box back and forth with the person carrying it and checks the fee against their file, so it must name them. `npm run fix:sample-letters` repaired the four letters already issued — **from the addressee's own office, never by reinterpreting the number**, because the row before those four carried a genuine lab id and reading it as an office would have moved a Dhaka letter to Barishal. |
| D131 | **A desk can be corrected by hand, and doing so is what clears `orgPostIsInferred`.** `/hr/listing/desks` over `setDesk()` in `lib/desk-service.ts`; superadmin only. | D124 flagged the guesses and said a re-seating screen was the next step; nothing could clear the flag until this existed, because `import:desks` only ever *fills* a null `orgPostId` and therefore cannot revisit a seat it got wrong. The scale is why it matters: **322 of the 480 seats are guesses, and every single one disagrees in rank** with the recorded designation, so `displayDesignation()` falls back to the recorded title for all of them and the desk's name is shown for the 158 confirmed rows alone. A wrong seat is not cosmetic — the desk decides which section a file routes to (D58) and which title a person reads on their own screen (D124). **Confirming is the act that clears the flag**, so the same click that fixes the routing fixes the title. Three rules, all enforced in the service rather than at the button: a post outside the employee's own office subtree is **refused**, because a file routed to their office would arrive at a desk in somebody else's — the fault `import:desks` releases seats to repair; a post at or over its sanctioned count is refused **by name**, listing who holds it, because the fix is to move them and a count does not say who — overridable, since 54 posts are already over and must stay editable; and a charge over a post that is already held **warns** rather than refusing, since additional charge normally covers a vacancy (D74) but somebody on leave is a real arrangement. The office→organogram-root mapping was lifted out of `import:desks`, which had the only copy. |
| D132 | **The Chemical Testing Wing's directorship changed hands**, 2026-09-13: Gazi Md. Nurul Islam released from Director (Chemistry), **Md. Khalilur Rahman holds it in additional charge** while remaining substantively Deputy Director (Chemistry) on the Organic Chemistry desk. | Found by signing in as him: `import:desks` had guessed him onto **Coordination Officer, grade 9, in Executive (Department of Director General)** — a grade-9 seat for a grade-6 officer in a wing he has nothing to do with, and one of the 322. `displayDesignation()` was doing its job, falling back to his recorded title because the ranks disagree, which is exactly how a bad seat stays invisible. Recorded in `import:hr-corrections` rather than by hand, because that script **asserts** Gazi onto post 728 and would have undone a hand-edit on its next run — so the earlier assertion had to be *changed*, not added beside. His designation is set to the qualified *Deputy Director (Chemistry)*; it was the bare *Deputy Director*, which names no wing. **This makes the third branch of `wingHeadForLab()` live**: head office's chemical sampling letters previously had a substantive Director to address and now resolve to an acting holder (D74, D94). Note the display consequence — with a charge recorded, `/api/me` shows the **acting** post's title, so his navbar reads *Director (Chemistry)*, which is D74's deliberate rule and not the substantive designation. |
| D133 | **The testing module: a wing head receives the samples, four rungs carry the work down, a testing officer records each reading with its own pass/fail, and the report collects up to three signatures before the wing head approves it.** `wing_head` and `testing_officer` are roles; `LabTestOrderMovement` and `LabTestReport` are the tables; `lib/labs/ladder.ts` is the Prisma-free half. | The client's spec, 2026-09-13, and step 9's first slice. **Not a variant of the CM chain** — stated by the client 2026-09-07 and it shapes everything: a file moves through the institution by seniority, while a laboratory runs four fixed rungs inside one wing, and the thing that moves is a `LabTestOrder` which carries no application column at all (D70). **The rungs skip what an office has not got**, which is the normal case and not the exception — eleven offices have only grade 9 in their CM section and a branch bench is often one Examiner — so `signaturePlan()` derives who signs from who exists: Examiner/AD/DD gives tested-checked-authorised; AD with no DD authorises; an Examiner alone tests *and* authorises his own report, because the alternative at such an office is a report nobody can sign and the wing head's approval is still a second pair of eyes. **Which wing a `wing_head` heads comes from their desk**, not a second column: head office's two Directors already sit in the wings they head, a branch head sits in Executive and therefore covers both, and a fact recorded once cannot drift. **A result is a value *and* a verdict** because the limit cannot be checked by machine — `LimitKind` splits four kinds a single column cannot (D61) and the text runs from "from 40 to 48" to "As per BDS 1149" to a manufacturer's declared value, so the officer judges and the system records; a parser guessing at those fails invisibly, which is the one failure mode a test result must not have. All pass → pass, one fail → fail, and an untested or inconclusive line **blocks submission by name**, the `reportGaps()` shape. **The FDO sees six stages and no rungs** (`lib/cm/lab-progress.ts`, the single named crossing): fee paid → awaiting submission → received by One Stop → received by the wing → under test → report approved. Who held an order and whether a draft was sent back stay inside the wing. **The counter's receive button was built here too** — `submitConsignment()` had existed since the sampling flow landed with **no caller at all**, so every sealed box in the system was uncollectable and the flow stopped dead one step after the letters. |
| D134 | **Urgent testing is the FDO's choice, made in the act of issuing the sampling letters, and the letter carries it as a badge — beside the tests that office is actually being asked to run.** `Application.isUrgent`, one flag for the whole file; `testFeeFor()` returns both totals and the choice picks one; the wing-head letter names each package's routed parameters instead of counting the catalogue's. | Three things the client asked for on 2026-09-13, and one that turned out to be wrong underneath them. **The letter told a wing head a number, not a list** — and the number was the *catalogue* package's, so application 26's Ceramic Tiles read "9 tests" on all three of its letters while Head Office was being sent 1, Faridpur 7 and Khulna 1. A wing head reads that letter to answer one question — can my bench run these — and a count cannot answer it. The parameters come from `LabTestOrderItem` through the D133 crossing, so the letter says what the order says. **Urgency did not exist on the CM side at all:** `LabTestOrder.isUrgent` was in the schema, nothing ever set it, and `testFeeFor()` summed only the normal fee — so the whole urgent price list (D99–D102, 4,767 parameters) was unreachable and every file was quietly normal. Decided at issue because that is the act that fixes the fee (D129) — the last moment the choice can be made and the first at which it can be charged for; the officer sees both totals and both turnarounds before choosing. **Whole file, not per office**, because `testFeePoisha` is one snapshot and one payment, and a file urgent at Dhaka and normal at Khulna has no single amount to demand. **The same write flips the orders**, or the letter would say জরুরি while the bench held a normal-priority order. **The urgent total is the sum of `urgentFeePoisha`, never the normal one doubled** — `priceUrgent()` writes the same figure as normal for a test that cannot be hurried (D102's `same_as_normal`, 106 of them), so doubling would charge for speed no bench can deliver. The badge is a border and bold type rather than a colour, because the letter is printed and a red panel photocopies to grey. |
| D135 | **A service gets a route prefix under `/workflow`, the shared desks stay at the top, and testing is one workflow rather than one per discipline.** `/workflow` (my files) · `/workflow/counter` · `/workflow/letters` are service-agnostic; `/workflow/cm/[id]/…` is CM's, `/workflow/metrology/…` and `/workflow/testing/[id]/…` follow. Plus two kernel rules: **no new service-specific column on `Application`**, and **nothing in `lib/workflow/` may import `lib/cm/`**. | The client's point, 2026-09-14: the wings do not share a workflow. CM runs from application to sampling approval and letter issue; the testing wings run from the letter to an approved report; CM resumes after it. Metrology's 12 services pass files the same way and do **entirely different work** at each desk. **RBAC was the proposed answer and is the wrong axis** — it answers "may this person do X", while the variation is in *what X is and what follows it*. Encoding process in a permission matrix produces roles like `metrology_pcr_stage3_approver`, and then authority and sequence are one tangled thing free to disagree. Authorisation is already adequate: `hasRole()` is the single question and D122 made roles multi-valued, so a new wing adds role **names**, which is data. **What is already right and must not be disturbed:** `lib/workflow/` imports nothing from `lib/cm/` — verified — so the movement kernel is genuinely generic, and `ServiceKind` was put on `Application` for exactly this. Twelve tables already hang off `applicationId` as work records rather than columns. **Testing is not split by discipline** because the discipline is not the unit of work: `LabTestOrder` is keyed on (office, sub-product), a branch office head is the wing head for **both** disciplines (his desk is in Executive, in no wing), and nothing in `ladder.ts` or `signaturePlan()` branches on it. Live proof: order `TO-TG2R-GS94-CMK` at Faridpur carries 7 tests that are chemical *and* physical, so a discipline-keyed route has no home for it. Discipline is a filter on a list, not a path segment. **The testing wing is a shared sub-workflow, not a service** — chemical and physical are one implementation (D133), and metrology will call the same module across the same narrow contract `lib/cm/lab-progress.ts` already models (D70). **Deferred deliberately:** `ApplicationState` is one flat enum of 30 CM values and `Application` still carries `factoryId`/`productId`; 12 metrology services would make that enum 130 values of mostly-inapplicable states. Splitting before a second real service exists is guessing at what varies — the same call D1 made about the monorepo. The blast radius is small and measured: **only 4 files type against `ApplicationState`**. The two rules above are what keep it small. |
| D136 | **`wing_head` and `testing_officer` stop being granted and are derived from the desk; and nobody signs their own test report — where the rung above the tester is empty the wing head gives the authorising signature and approves in one act.** **Amends D133.** | The client's model, 2026-09-14, and it fixed a module that could not run. **A granted role that duplicates a derivable fact is two sources of truth free to disagree** — D122's reasoning about `User.role`, applied again. The proof it was wrong: every desk was in place, nobody held the roles, and all 23 offices had no wing head and no testing officer. After the change, **23 of 23 offices resolve both, with zero grants.** **Head office answers from the desk, a branch from the `office_head` role.** Whoever holds the Director post in `Executive (Physical Testing Wing)` or `Executive (Chemical Testing Wing)` heads it, substantive or in additional charge (D74) — Mobin Ul Islam [physical] and Khalilur Rahman acting [chemical]. The test is on a *testing* wing and not merely Executive, because head office's own `office_head` is the Director of the Certification Marks Wing and heads no laboratory. **A branch keys on the role, not the Executive desk, because only 11 of 23 office heads sit in their office's Executive section** — the rest are in CM, in Metrology, one in a Physical Lab, and two hold no desk at all. Deriving from the desk would have left twelve offices headless; the role is 23 of 23. **And a branch head always covers both disciplines regardless of the unit he sits in**: reading his unit made Pabna's head `physical` only, because he is seated in *Physical Lab, Pabna*, which would have refused a chemical order at his own office. **Every Examiner and every Assistant Director is a testing officer by the desk they hold.** `isTestingRung()` is the one place that is decided, shared by `enterResult()`, `submitReport()` and the screen — so an office is never one forgotten grant away from a dead bench. **The signature table changes in one row.** D133 had the Examiner authorise his own report at a one-bench office and defended it on the grounds that the alternative is a report nobody can sign; the answer is that there is somebody, and it is the officer who was going to approve it anyway. Head office always staffs Director → DD → AD → Examiner, so its reports are signed three rungs down and the Director only approves; at Pabna and Dinajpur, which staff no Examiner and nothing above the AD, the office head now signs and approves. `approveReport()` stamps the authorising signature in the same act, and recomputes the plan rather than trusting the draft. **A person holding two desks may take two steps on one order** — Khalilur authorising as Deputy Director (Chemistry) and approving as acting Director (Chemistry) — the client's explicit call: the desk he acts from is what counts. Desk *switching* is not built; until it is, authority follows every desk he holds at once. |
| D137 | **The wing-head letter and the work order are one instruction, so they live in one module: `/labs/orders` becomes `/workflow/work-order`, and each order links to the letter it came from.** Amends D135's placeholder `/workflow/testing/`. `lettersForWorkOrders()` in `lib/cm/letter-inbox.ts` is the crossing; `workOrderCountForViewer()` puts the tab in the `/workflow` navbar on the same terms as the counter and the inbox. | The client's own reading, 2026-09-15: *"letters issued by FDO to wing head is found at /workflow/letters and work order is found at /labs/orders. Why is that? This is a single package. That letter is the work order."* He is right, and the split was an artefact of the order table having been built inside the lab module before there was anywhere else to put it. **The line `/labs` actually draws is maintenance against work** — the catalogue, an office's coverage, the routing map and the registry of laboratories are tables somebody maintains; a work order is a file somebody works, and every other file in the institution is worked in `/workflow`. **Named for the thing, not the activity** (`work-order`, not `testing`): the officer knows it by the letter he was sent, and naming the route after the paper is what makes the two obviously one. **It sits beside the shared desks rather than under a service prefix**, because metrology's samples will arrive on the same bench — D135's reasoning about testing being a shared sub-workflow, unchanged. **The crossing belongs on the CM side.** Reaching from an order to its letter means order → specimen → `SampleRegistration` → consignment → application, and `SampleRegistration` is the cut (D71) whose own comment is that nothing lab-facing reads it — so the join lives beside the letter rules and `lib/labs/board.ts` stays blind by construction. What comes back is an id and a letter number, never a company. One query for the orders and one for the letters, whatever the board's size. **The link is offered only to the letter's addressee**, which is the rule `internalLetterFor()` already enforces: a wing-head letter belongs to the officer named on it, so offering it to a bench would be a button that opens on a refusal. An Assistant Director holding the order sees no link — he has the order, which says the same thing. **The reverse link is not built**, because one letter can cover several packages in one box and therefore several orders, and whether it should lead to the board or list each order is the client's call. Two things were corrected in passing: the board now resolves its office through `actorFor()` like every other `/workflow` screen rather than reading `Employee.officeId` directly, and `ladderHealth()` stopped telling an office to grant roles that D136 abolished — it names the empty desk instead. |
| D138 | **A wing-head letter gets no inbox row of its own: it *is* the work order, so `/workflow/work-order` carries its substance and `/workflow/letters` becomes the One Stop counter's inbox alone.** `lettersForViewer()` and `letterCountForViewer()` filter to `kind: "one_stop"`; `lettersForWorkOrders()` returns the letter's operative facts rather than an id; the letter page's back link follows its kind. **Completes D137.** | The client, 2026-09-15, having seen D137's link and found it a half-measure: *"I think the letter and work order section should not be different. Make the related letter available on the work-order section."* He is right, and the link was the timid version of the same idea. **One instruction must have one destination.** The FDO's letter says *test these parameters on this package*, and `submitSamplingPlan()` has already written that sentence as a `LabTestOrder` with a state and a holder — so a tab for each left the wing head to notice that his two lists were the same list, keyed differently: one letter per box against one order per package, so a two-package box appeared once on one screen and twice on the other. **What moved is the substance, not just the link.** The inbox row carried facts the board did not — the box code and seal, the jar count, whether the box had been handed in, whether the testing fee was settled, and the urgent basis — and deleting the row without them would have left a wing head waiting on a box the counter was refusing over an unpaid fee with nothing on screen saying so (D129). Those are now a card on the order, and the unpaid-fee warning is on the board card as well, which is where the waiting happens. **The paper stays a page.** `internalLetterFor()` is untouched and still serves both kinds: a letter is a printed document, it has a PDF route and a print stylesheet, and a workspace is not where you print from. Only the *listing* changed. **The counter keeps its inbox** because a counter letter has no work order behind it — the clerk takes the box and tests nothing — which is also the line that says why the wing head's did not need one. Renamed *Counter letters* so the tab says what it now holds. **The access rule is unchanged** (D130): the letter still decides who may read it, and the work order only ever shows the letter to the officer it is addressed to. |
...
**Most of that was answered 2026-09-13 and built as D133.** Who holds an order:
a wing head receives it and passes it down four rungs, choosing a person at each.
Results: per result-bearing leaf per specimen (D61's rule), entered by a
`testing_officer`, correctable while the order is on his bench and frozen once
the report goes up. The report: `LabTestReport`, up to three signatures decided
by who the office staffs, approved by the wing head and numbered only then. The
test fee was settled earlier by D129.

**Urgent testing landed 2026-09-13 as D134.** The FDO chooses the basis in the
act of issuing the sampling letters, the fee is charged at that rate, the file's
orders are flipped with it, and the letter carries it as a badge. That closed a
gap nobody had noticed: `LabTestOrder.isUrgent` had existed since the samples
schema and **nothing had ever set it**, so the urgent price list built over
Sessions 5–12 was unreachable from the CM side and every file was quietly
normal. The same letter now names the parameters routed to that office instead
of counting the catalogue package's, which was wrong on every file whose tests
split across two offices.

**Still open:** how a third-party referral (D65) is recorded — the office is
accountable and its own examiner enters the result, but nothing yet marks *that*
a sample went outside; whether a report can be revised after approval; and what
the printed report looks like as a document (there is no signature image on
`Employee`, only on `DirectorGeneral`, so the signature blocks render as
name + designation + date, the way the office order does).
