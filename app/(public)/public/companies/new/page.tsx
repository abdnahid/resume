...
export default async function NewCompanyPage({
  ...
  searchParams: Promise<{ parent?: string; next?: string }>;
}) {
  const viewer = await requireClient("/public/companies/new");
  const { parent, next } = await searchParams;

  // Where the wizard sends them when it is done. The apply page passes
  // itself, so a company set up mid-application returns to the application.
  // `safeNext()` discards anything that is not app-relative.
  const done = safeNext(next);
  ...
  return (
    <div className="mx-auto w-full max-w-[900px] px-5 py-14 lg:px-10">
      ...
      <div className="mt-10">
        <ProfileWizard parentId={parentId} parentName={parentName} returnTo={done} />
      </div>
    </div>
  );
}
